...
parser = argparse.ArgumentParser(description="Keras combined wavelet-curvelet software")
...
# Test data
C_n, C_s = C_N.iloc[Test], C_S.iloc[Test]
dfs_C_ = np.asarray(pd.concat([C_n, C_s]))
N, S = DF.iloc[Test], DF1.iloc[Test]
dfs_W_ = np.asarray(pd.concat([N, S]))
...
# Save results
np.savetxt(f"../Data/Results/Nonlin_Preds_CW_{align_val}.csv", p, fmt='%.18e', delimiter=' ')
np.savetxt(f"../Data/Results/Nonlin_Prob_CW_{align_val}.csv", l, fmt='%.18e', delimiter=' ')
c1 = confusion_matrix(y1, p)
np.savetxt(f"../Data/Results/Nonlin_CM_CW_{align_val}.csv", c1, delimiter=' ')
