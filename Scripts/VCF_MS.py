...
parser.add_argument('filename', type=str, help= 'Name of the vcf file that is placed on the Data folder')
...
with open(f'../Data/{fname}.vcf') as f:
    lines = f.readlines()

with open('../Data/MissSWP_test.txt') as f:
    lines1 = f.readlines()
...
for i in range(len(M)):
    ...
    with open('../Data/VCF/output_'+str(i)+'.ms','w') as f:
        f.write(lines1[0])
        f.write(lines1[1])
        f.write(lines1[2])
        f.write(lines1[3])
...
pd.DataFrame(P).to_csv("../Data/VCF/Starting Positions of the samples.csv")
