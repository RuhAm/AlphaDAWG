...
parser = argparse.ArgumentParser(description="Keras curvelet software")
...
# Load data
DF = pd.read_csv(f"../Data/Curvelets/Curvelets_neut_{align_val}_resized_.csv", header=None)
DF1 = pd.read_csv(f"../Data/Curvelets/Curvelets_sweep_{align_val}_resized_.csv", header=None)
num_rows = DF.shape[0]
...
dfs = np.asarray(pd.concat([N, S]))
N_, S_ = DF.iloc[Test], DF1.iloc[Test]
dfx = np.asarray(pd.concat([N_, S_]))
...
# Prepare test labels
y1 = np.concatenate([np.zeros(test_observations), np.ones(test_observations)])
results = model.evaluate(X_test_std_sim, y1)
l = model.predict(X_test_std_sim, verbose=1)
p = np.where(l > 0.5, 1, 0)

# Save results
np.savetxt(f"../Data/Results/Nonlim_Pred_Crv_{align_val}.csv", p, fmt='%.18e', delimiter=' ')
np.savetxt(f"../Data/Results/Nonlim_Prob_Crv_{align_val}.csv", l, fmt='%.18e', delimiter=' ')
c1 = confusion_matrix(y1, p)
np.savetxt(f"../Data/Results/Nonlim_cm_crv_{align_val}.csv", c1, delimiter=' ')
